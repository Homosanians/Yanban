...
// --- a narrow DI root ------------------------------------------------------------
//
// NOT `AddInfrastructure()`. That registers everything the API needs, including IActivityRecorder,
...
// does need object storage now (the janitor deletes from S3), but only the internal client. It
// never mints presigned URLs, so there is no second client to build.
...
// apply the same migration on startup would be a bug.
