...
/// read, which should not carry recipients and payloads.</para>
///
/// <para>Claimed by status, not by a sequence cursor: <c>WHERE status = 'Pending' ... FOR UPDATE
/// SKIP LOCKED</c> re-scans on every poll, so a row that commits late is picked up whenever it
/// becomes visible. That is why this outbox needs no grace window, unlike
/// <see cref="Yanban.Domain.Entities.ActivityLog"/>'s consumer.</para>
/// </summary>
public class OutboxMessage
{
    ...
    /// <summary>
    /// jsonb. Everything the worker needs to render the mail without re-reading the domain, which
    /// would be a stale read anyway (the card may have moved on since).
    ///
    /// <para>Nulled once sent: a confirmation payload carries a live token, and a spent message
    /// should not keep a working credential at rest.</para>
    /// </summary>
