...
///
/// <para>Only overrides are stored. A user who never touches a toggle has no rows at all;
...
/// treats NULLs as distinct in a unique index, so the global row's uniqueness is enforced by a
/// second, filtered unique index rather than by the composite one (see the EF configuration).</para>
/// </summary>
